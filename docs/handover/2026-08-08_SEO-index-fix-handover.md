...
> ✅ **2026-08-18 実施済み。結果は `2026-08-18_breadcrumb-structured-data-fix.md` §E を参照。**
> 要点: 登録済み 19 → **25**（+6）／未登録 47 → **27**（−20）。
> 特に「検出 - インデックス未登録」が 14 → **1**（−13）。
> この14件は全て末尾スラッシュ付きURLで、`38b34ac` が発生源を断った。
> 以下は当時の計測手順の記録として残す。
