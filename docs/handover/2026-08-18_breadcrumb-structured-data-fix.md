...
⚠️ **この2行は 2026-08-26 に訂正済み。** 上記は全件列挙による確認ではなく見積もりだった。
08/21 時点で16件を全件列挙したところ、サイトマップ掲載の実在ページが2件
（`/free-trial`・`/support/help`）残っていた。
詳細は §I-2 の `<finding type="issue" date="2026-08-26">` を参照。
...
<task priority="2">
### I-2. インデックス数値の再測定（2026-09-10 頃）

§E の数値には a8f1bfd（08/18）と `/blog/digital-namecard` の転送が反映されていない。
**2026-08-26 に一度確認したが、判定できなかった**（下記 <known-fact> 参照）。
**2026-09-10 頃**にもう一度「ページのインデックス登録」を見て、以下を確認する。

- 「登録済み」が25から増えているか
- 「クロール済み - インデックス未登録」が16から動くか
- 「ページにリダイレクトがあります」に `/blog/digital-namecard` が現れるか
  （現れるのが正常。是正済みの印であって不具合ではない）
- **`/free-trial` が「クロール済み - インデックス未登録」から外れているか**
  （下記 <finding> 参照。外れていなければ原因調査に入る）
</task>

<known-fact date="2026-08-26">
### 中間測定（判定不能）

Super が GSC「ページのインデックス登録」を閲覧。**データの最終更新日 2026/08/21**。
a8f1bfd（08/18）から3日分しか反映されておらず、**効果は判定できない**。

| 区分 | 08/05（修正前） | 08/14 | 08/21 |
|---|---|---|---|
| 登録済み | 19 | 25 | 25 |
| 未登録 | 47 | 27 | 26 |

未登録26件の内訳（検算 1+16+3+4+2 = 26 ✓）

| 理由 | 08/14 | 08/21 |
|---|---|---|
| 検出 - インデックス未登録 | 1 | 1 |
| クロール済み - インデックス未登録 | 18 | 16 |
| ページにリダイレクトがあります | 3 | 3 |
| 見つかりませんでした（404） | 3 | 4 |
| robots.txt によりブロック | 2 | 2 |

「ページにリダイレクトがあります」3件の内訳（08/21 時点・全件列挙）:
`https://www.sns-share.com/` / `http://sns-share.com/` /
`https://sns-share.com/digital-namecard/for-executives/`
→ **`/blog/digital-namecard` はまだ現れていない。** 3日分のデータのため想定内。

「見つかりませんでした（404）」4件の内訳（08/21 時点・全件列挙）:
`https://sns-share.com/upload/1422752243.m3u8`（最終クロール 08/22）/
`https://sns-share.com/index.html`（08/19）/
`https://app.sns-share.com/pricing`（08/09）/
`https://app.sns-share.com/_next/static/css/c51efbc8a1b8ef91.css?dpl=...`（07/16）
→ §E で「`/blog/category` が有力」と推測した件は、この一覧に含まれていない。
   当時の推測は現時点のデータでは裏付けられていない。
</known-fact>

<finding type="issue" date="2026-08-26">
### 中身のある2ページが「クロール済み - インデックス未登録」に残っている

**16件を1件ずつ全件列挙して数えた結果**（§E は列挙せず見積もっていた）:

| 区分 | 件数 | 内訳 |
|---|---|---|
| サイトマップ掲載の実在ページ | **2** | `/free-trial`（最終クロール 07/31）、`/support/help`（03/03） |
| 末尾スラッシュ付き | 1 | `/legal/privacy/` |
| 静的アセット | 1 | `/_next/static/css/fcbff10c...` |
| www. 付き重複 | 5 | `/blog/digital-namecard/sns-integration`、`/support/help`、`/digital-namecard`、`/legal/privacy`、`/digital-namecard/about` |
| app.sns-share.com（別リポジトリ `~/Projects/share` の管轄） | 7 | `/legal/transactions`、`/company/service`、静的CSS×3、`/api/manifest`、`/pwa/favicon.ico` |
| 計 | **16** | ✓ |

**`/free-trial` は sitemap.ts で priority 0.9（主要導線）を与えている無料トライアル申込ページ。**
ここが検索結果に出ていない。

技術的な阻害要因は見当たらない（Super がコードで実測）:
- `src/` 配下に `noindex` の記述は1件もない
- `src/app/free-trial/page.tsx` に title / description / openGraph を設定済み
- `src/app/sitemap.ts:80` に `{ path: "/free-trial", ... priority: 0.9 }` として掲載済み

**原因は未確定。断定しないこと。** 2026-09-10 の測定で解消していなければ調査に入る。

⚠️ **§E の「残る18件はほぼ全て『もともと載る必要のないもの』」「中身のあるページは概ね解消したと見てよい」
は、全件列挙による確認ではなく見積もりだった。** 上表のとおり実在ページが2件残っている。
以降、件数の性質を語るときは必ず全件列挙してから書くこと。
</finding>
