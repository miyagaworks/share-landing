...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 送信元アドレスを設定
    // 通常はVerify済みのドメインを使用するが、デバッグ用に一時的にResendのデフォルトドメインを使用
    const from = "onboarding@resend.dev"; // Resendが提供する検証済みドメイン
...
<div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
  ...
  <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e0e0e0; font-size: 12px; color: #666;">
    ...
    <div style="margin-top: 10px; font-size: 11px; color: #999;">
      ...
      運営: ビイアルファ株式会社</p>
      <p>
        <a href="https://sns-share.com/legal/privacy" style="color: #4A89DC; text-decoration: none;">プライバシーポリシー</a> | 
        <a href="https://sns-share.com/legal/terms" style="color: #4A89DC; text-decoration: none;">利用規約</a>
      </p>
    </div>
  </div>
  ...
  <p style="color: #6B7280; font-size: 13px; margin-top: 30px; padding-top: 10px; border-top: 1px solid #e2e8f0;">
    ...
    try {
      ...
      const adminEmailResult = await resend.emails.send({
        from: `${siteName} <${from}>`, // Resendのデフォルトドメイン
        to: [to],
